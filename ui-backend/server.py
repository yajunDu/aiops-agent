...
def list_services():
    ...
    for p in pods_data:
        ...
        # 状态判定（cpu_val 是绝对核数*100，train-ticket Pod limit≈0.5核，>40 即高负载告警）
...
def chat_with_qwen(req: ChatRequest):
    """智能对话 - 分意图处理：状态查询 / Agent 诊断 / 通用问答"""
    try:
        from smart_chat import smart_chat
        result = smart_chat(req.message, req.history)
        return result
    except Exception as e:
        import traceback
        traceback.print_exc()
        return {"reply": f"❌ Chat 失败: {e}", "error": str(e)}
